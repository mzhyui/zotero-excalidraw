...
if (!Zotero.ZoteroExcalidraw.Utils) Zotero.ZoteroExcalidraw.Utils = {};

Zotero.ZoteroExcalidraw.Utils = Object.assign(Zotero.ZoteroExcalidraw.Utils, {
	init() {
		Zotero.ZoteroExcalidraw.Logger.log('Zotero.ZoteroExcalidraw.Utils inited.');
	},
  ...
  getCurrentUsername() {
    return Zotero.ZoteroExcalidraw.Prefs.get('sync.server.username', Zotero.Users.getCurrentUsername());
  },
  ...
  async loadAttachmentImg(note) {
    ...
    if (imgs.length === 0) {
      Zotero.debug('zotero-excalidraw@loadAttachmentImg no has imgs.')
      return {
        id: note.id,
        note: noteContent
      }
    ...
    Zotero.debug('zotero-excalidraw@loadAttachmentImg: ' + doc.body.innerHTML.length)
    return {
      id: note.id,
