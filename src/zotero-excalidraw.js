...
Zotero.ZoteroExcalidraw = Object.assign(Zotero.ZoteroExcalidraw, {
	...
	init({ id, version, rootURI }) {
		...
		Zotero.getMainWindow().ZoteroPane.viewItems = async function (items, event) {
			Zotero.ZoteroExcalidraw.Logger.log('ZoteroPane.viewItems');
			for (let i = 0; i < items.length; i++) {
				let item = items[i];
				if (item.isRegularItem()) {
					// Prefer local file attachments
					// CHANGE: 'yield' becomes 'await'
					let attachment = await item.getBestAttachment();
					if (attachment) {
						// CHANGE: 'yield' becomes 'await'
						await Zotero.getMainWindow().ZoteroPane.viewAttachment(attachment.id, event);
						continue;
					}
					...
					if (!uri) {
						...
						if (doi) {
							...
							if (doi) {
								uri = "http://dx.doi.org/" + encodeURIComponent(doi); // ZoteroPane.encodeURIComponent is unnecessary, standard JS works
							}
						}
					}
					...
					if (item.attachmentContentType === 'application/excalidraw') {
						...
						// CHANGE: 'yield' becomes 'await'
						await Zotero.getMainWindow().ZoteroPane.viewAttachment(item.id, event);
					}
				}
			}
		};
